...
\title{SNORT\textsuperscript{\textregistered} Users Manual\\2.9.5.5}
...
\texttt{<path>} specifies the directory for snort to creat the socket. If relative path is used, 
...
attachment filenames extracted from the Content-Dispostion header
...
\hline
\texttt{toggle} & For every state specifed, sets the specified state if the state is unset and unsets it if the state is set.\\
\hline
...
\end{verbatim}
  ...
  unified2 record allows an interpretting application to skip past and
...
\end{verbatim}
  ...
  (not including the Serial Unified2 Header itself) upto the next 
...
\subsection{Unfied2 IDS Event IP6}
...
\subsection{Unfied2 IDS Event (Version 2)}
...
\item Data (Unified2 Extra Data)
    ...
    need to be interpreted as if they were comming off the wire.
...
\item Protocol
    ...
    Transport protcol of the alerting packet. One of: ip, tcp, udp, or
    icmp.
...
\begin{note}
Note that you'll only obtain Would Have Dropped on rules which are set to drop
while Snort is running in inline-test mode.
\end{note}
